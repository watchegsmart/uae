...
function goToPage(pageId) {
    ...
    // Update active state on header icon buttons
    ['nav-cart','nav-fav'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.classList.remove('active');
    });
    if (pageId === 'cart-page') { const el = document.getElementById('nav-cart'); if(el) el.classList.add('active'); }
    else if (pageId === 'favorites-page') { const el = document.getElementById('nav-fav'); if(el) el.classList.add('active'); }
    
    // Scroll content to top on page switch
...
function goToSlide(index) {
    ...
    // Use negative for RTL compatibility if needed, but the CSS is direction: ltr
...
// Fix: scroll focused input into view when keyboard opens (iOS/Android)
