...
// ===== KEYBOARD VIEWPORT FIX - إصلاح مشكلة لوحة المفاتيح على الهواتف =====
// عندما تظهر لوحة المفاتيح على iOS يتقلص الـ viewport ويتحرك المحتوى
    ...
    window.addEventListener('resize', () => {
        ...
        if (appContainer) {
            if (currentHeight < originalHeight * 0.75) {
                // لوحة المفاتيح ظاهرة - ضبط الارتفاع
                appContainer.style.height = currentHeight + 'px';
            } else {
                // لوحة المفاتيح مخفية - استعادة الارتفاع الأصلي
                appContainer.style.height = '';
            }
        }
    });
    
    // عند الضغط على حقل إدخال - تمرير الحقل ليكون مرئياً
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    startHeroCarousel();
    setupOTPInputs();
    handleKeyboardVisibility(); // إصلاح مشكلة لوحة المفاتيح
});
